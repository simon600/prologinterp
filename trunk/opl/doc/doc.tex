...
\section{Ogólny zarys}
	...
	\item Interpreter zezwala na zadawanie zapytan postaci $X=f(X)$. W tym wypadku zostanie wypisane podstawienie $X = f(X)$.
 	\item Interpreter nie zezwala na uzycie funktorów bezargumentowych. 	
	\end{enumerate}
...
\section{Budowa interpretera}
	\subsection{Podzia³ logiczny}
		Projekt sk³ada siê z kilku modu³ów oraz jednego pliku "`g³ównego"', w którym znajduje siê g³ówna pêtla programu.
		\begin{itemize}
			\item \textbf{Parser:}\\
				Modu³ ten zawiera implementacjê parsera jêzyka \textit{Prolog} oraz udostêpnia funkcje s³u¿¹ce do parsowania zarówno zapytañ,
				jak i plików z bazami danych.  
			\item \textbf{Lexer:}\\
			  Wykorzystywany przez modu³ parsuj¹cy do dzielenia zadanego wejœcia na tokeny. 
			\item \textbf{Unificator:}\\
			  Jednostka ta udostêpnia zestaw funkcji pozwalaj¹cych na unifikowanie termów oraz udostêpnia szereg funkcji pomocniczych
			  do pracy na termach.
			\item \textbf{Evaluator:}\\
				Modu³ \texttt{Evaluator} jest podstawow¹ jednostk¹ interpretuj¹c¹ programy (zapytania) w programie. Udostêpnia funkcjê 
				s³u¿¹c¹ do interpretacji abstrakcyjnych drzew rozbioru, zwróconych przez funkcje z modu³u \texttt{Parser}.
			\item \textbf{Types:}\\
			  Modu³ zawieraj¹cy deklaracje typów potrzebnych do konstrukcji abstrakcyjnych drzew rozbioru. 
			\item \textbf{Opl:}\\
			  Jednostka kompilacji, bêd¹ca zarazem interpreterem samym w sobie. 
		\end{itemize}

	Czêœæ z modu³ów nie ma osobno zdefiniowanego interfejsu (sygnatury); jest tak dlatego, i¿ upubliczniany jest pe³en zakres funkcji
	zaimplementowanych w tych modu³ach.

	\subsection{Podzia³ fizyczny}
		\begin{itemize}
			\item \texttt{opl.ml}:\\ 
				Plik z g³ówn¹ pêtl¹ programu.
			...
			\item \texttt{lexer.mll}:\\
				Plik z wejœciem dla generatora lekserów \texttt{ocamllex}.
			\item \texttt{parser.mly}:\\
				Plik z wejœciem dla generatora parserów \texttt{ocamlyacc}.
			\item \texttt{types.ml}:\\
				Plik z implementacj¹ modu³u \texttt{Types}.
		\end{itemize}	
		
		W plikach \texttt{*.bat} znajduj¹ siê proste pliki wsadowe systemu \textit{Windows}, dziêki którym mo¿na 
		skompilowaæ projekt zarówno w trybie \textit{debugowania} (pliki \texttt{debug.bat}, \texttt{undebug.bat}), jak
		i w zwyk³ym (plik \texttt{make.bat}). Jednoczeœnie w sk³ad projektu wchodz¹ tak¿e skrypty kompiluj¹ce 
...
\section{Dodatek A - operatory}
	...
	\item \textbf{Operatory infixowe: }
		...
		$<$= & sprawdza czy wyr. arytmetyczne po lewej stronie daje mniejszy lub równy wynik\\\hline
		$>$= & sprawdza czy wyr. arytmetyczne po lewej stronie daje wiêkszy lub równy wynik\\\hline
